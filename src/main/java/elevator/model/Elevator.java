...
public class Elevator {
    ...
    private void moveOneFloor(Division division, int targetFloor) throws InterruptedException, IllegalArgumentException{
        // State dir = targetFloor > currentFloor ? State.UP : State.DOWN;
    ...
    public void processQueue(Division division) throws InterruptedException {
        while (!destinationQueue.isEmpty()) {
            ...
            if (currentFloor == target) {
                // Already there (e.g. pushed a duplicate before removal fixed it)
                destinationQueue.pollFirst();
                handleFloorStop(division, currentFloor);
                continue;
            }

            // Elevator stopped - either error or let out people
            ...
            // After each step, check if this floor is queued (en-route stop)
    ...
    /** Returns an unmodifiable view of the current occupants. */
    public List<Entity> getOccupants(){
        return java.util.Collections.unmodifiableList(occupants);
    }

    /** Returns the current destination queue (head = next target). */
    public LinkedList<Integer> getDestinationQueue(){
        return new LinkedList<>(destinationQueue);
    }

    /** Returns the current direction of travel, or IDLE if the queue is empty. */
    ...
    public void printStatus() {
        System.out.printf("Status windy: Piętro = %d | Ludzie w windzie = %d/%d | Kolejka pięter = %s | Status = %s%n", currentFloor, occupants.size(), maxCapacity, destinationQueue, getDirection());
    }
