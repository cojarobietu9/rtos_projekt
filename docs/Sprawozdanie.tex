...
\item[\texttt{enqueueDestination(int floor)}]
  ...
  \item[\texttt{pushDestinationFront(int floor)}]
    Wstawia piętro z wysokim priorytetem (przed wszystkimi wpisami bez priorytetu ujemnego). Stosowane, gdy pasażer wsiada i jego piętro docelowe powinno być obsłużone wcześniej.


  \item[\texttt{handleFloorStop(Division division, int floorNum)}]
    Obsługuje postój windy: najpierw wysiada pasażerów z bieżącym piętrem docelowym, następnie wsiada oczekujących (o ile nie przekroczono pojemności), dodaje ich piętra docelowe priorytetowo do kolejki i symuluje czas postoju (\texttt{IOwaitMs}).

  \item[\texttt{isFull()}, \texttt{isEmpty()}]
    Sprawdzają, czy kabina jest zapełniona do maksymalnej pojemności lub pusta.

  \item[\texttt{getDirection()}]
    Zwraca przewidywany kierunek ruchu (\texttt{UP}/\texttt{DOWN}/\texttt{IDLE}) na podstawie pierwszego wpisu w kolejce.
\end{description}
...
Reprezentuje pion budynku - zbiór pięter obsługiwanych przez jedną windę. Łączy obiekty \texttt{Floor} z obiektem \texttt{Elevator} i udostępnia wysokopoziomowe API do wywoływania windy.

\textbf{Najważniejsze metody:}

\begin{description}[leftmargin=1cm, style=nextline]
  \item[\texttt{Division(String name, int startFloor, int endFloor, Elevator elevator)}]
    Konstruktor tworzy listę pięter od \texttt{startFloor} do \texttt{endFloor} (włącznie). Rzuca \texttt{IllegalArgumentException}, gdy \texttt{endFloor < startFloor}.

  \item[\texttt{callElevator(int originFloor, int destinationFloor)}]
  \label{division:callElevator}
    Główna metoda API: tworzy pasażera na piętrze \texttt{originFloor} (przez \texttt{Floor.spawnEntity}) i dodaje to piętro do kolejki windy (\texttt{Elevator.enqueueDestination}). Zwraca referencję do nowo utworzonego pasażera.

  \item[\texttt{getFloor(int floorNumber)}]
    Zwraca obiekt \texttt{Floor} dla podanego numeru piętra. Rzuca \texttt{IndexOutOfBoundsException} przy numerze spoza zakresu.

  \item[\texttt{getMinFloor()}, \texttt{getMaxFloor()}]
    Wyznaczają odpowiednio najniższe i najwyższe piętro w pionie, korzystając z naturalnego porządku obiektów \texttt{Floor}.

  \item[\texttt{printStatus()}]
    Wypisuje na standardowe wyjście aktualny stan wszystkich pięter oraz windy - pomocnicze narzędzie diagnostyczne.
\end{description}
...
\textbf{Pakiet:} \texttt{GUI}

Główne okno aplikacji zbudowane na bazie \texttt{JFrame} (Swing). Odpowiada za wizualizację stanu symulacji w czasie rzeczywistym oraz interakcję z użytkownikiem. Komunikacja z warstwą modelu odbywa się przez interfejs \texttt{ElevatorListener} - winda wywołuje \texttt{updateUIState()} po każdym ruchu i postoju.

\textbf{Układ interfejsu:}
\begin{itemize}
  \item \textbf{Północ} (\texttt{BorderLayout.NORTH}) - panel parametrów symulacji (zakresy pięter, prędkość, czas postoju, pojemność) z przyciskiem \emph{Zastosuj i restartuj},
  \item \textbf{Centrum} - dwie kolumny: lista pięter (z zaznaczeniem aktualnej pozycji windy) i panel stanu windy (piętro, prędkość, kierunek, pasażerowie, kolejka),
  \item \textbf{Południe} - panel dodawania pasażera ręcznego oraz przyciski sterowania symulacją.
\end{itemize}
...
\begin{description}[leftmargin=1cm, style=nextline]
  \item[\texttt{initSimulationFromInputs()}]
    Odczytuje wartości ze spinnerów, tworzy nowy obiekt \texttt{Elevator} i \texttt{Division}, rejestruje listener GUI oraz dodaje domyślnych pasażerów startowych.

  \item[\texttt{startSimulationThread()}]
    Uruchamia wątek symulacji (jeśli nie jest aktywny), który wywołuje \texttt{Elevator.handleFloorStop} oraz \texttt{Elevator.processQueue}.

  \item[\texttt{stopSimulation()}]
    Ustawia flagę \texttt{stopRequested} i przerywa wątek symulacji przez \texttt{Thread.interrupt()}.

  \item[\texttt{spawnRandomPassenger()}]
    Losuje parę pięter (różnych) w ramach aktualnego zakresu \texttt{Division} i wywołuje \texttt{callElevator}. Jeśli winda stoi i symulacja jest aktywna, automatycznie wznawia wątek.

  \item[\texttt{addManualPassenger()}]
    Pobiera wartości ze spinnerów \emph{Z piętra} i \emph{Na piętro} i wywołuje \texttt{callElevator}. Waliduje, że piętra są różne.

  \item[\texttt{updateUIState()}]
    Aktualizuje wszystkie etykiety i listę pięter za pomocą \texttt{SwingUtilities.invokeLater}, zapewniając wywołanie na wątku EDT.
\end{description}
...
\begin{itemize}
  ...
  \item Opcjonalnie: \textbf{Maven} lub \textbf{Gradle} do zarządzania budowaniem projektu (projekt nie posiada pliku \texttt{pom.xml} ani \texttt{build.gradle} - poniżej podano ręczną kompilację).
\end{itemize}

\subsection{Kompilacja i uruchomienie z wiersza poleceń}

Zakładając strukturę katalogów zgodną z archiwum \texttt{src.zip}:

\begin{lstlisting}[language=bash, caption={Kompilacja projektu}]
cd src/main/java
...
\begin{enumerate}
  \item Otwórz projekt w wybranym środowisku (IntelliJ IDEA, Eclipse, NetBeans).
...
Po uruchomieniu aplikacji dostępny jest panel \emph{Parametry symulacji} pozwalający na zmianę:
...
% TODO: Uzupełnić podsumowanie projektu.
% Sugerowane punkty do opisania:
%   - Wnioski z realizacji projektu
%   - Napotkane trudności i sposób ich rozwiązania
%   - Możliwe kierunki dalszego rozwoju (np. obsługa wielu wind, algorytmy SCAN/LOOK)
%   - Ocena zastosowanych rozwiązań technicznych

\textit{[Miejsce na podsumowanie - do uzupełnienia przez autora.]}
